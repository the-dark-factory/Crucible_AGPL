...
--  Unstamped source sha256: 72465836fb6b3788041e80ae85c991e3d74ca81e3cea90bbd76f6e543fbdbb42
--
with Json_Scan_Pkg;
...
package Rail_Config_Pkg with SPARK_Mode is
   ...
   type Rail_Kind is (Rail_Prover, Rail_Model, Rail_Unknown);
   ...
   function Dot_After (Line : String; From : Positive; Last : Natural) return Natural
     with Pre  => (Line'First = 1 and then Line'Length <= Max_Line and then From >= 1 and then Last <= Line'Last and then From <= Last + 1),
   ...
   function Rail_Of (Line : String) return Rail_Kind
     is ((if not Contents (Line, Rail_Key).found then Rail_Unknown
          elsif Json_Scan_Pkg.Equals_Literal (Line, Contents (Line, Rail_Key), Prover_Word) then Rail_Prover
          elsif Json_Scan_Pkg.Equals_Literal (Line, Contents (Line, Rail_Key), Model_Word) then Rail_Model
          else Rail_Unknown))
   ...
   function Sovereign_Ok (Line : String) return Boolean
     is (Contents (Line, Sovereign_Key).found and then
         (Json_Scan_Pkg.Equals_Literal (Line, Contents (Line, Sovereign_Key), True_Word) or else
          Json_Scan_Pkg.Equals_Literal (Line, Contents (Line, Sovereign_Key), False_Word)))
     with Pre  => Line'First = 1 and then Line'Length <= Max_Line,
