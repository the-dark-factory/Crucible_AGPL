...
--  Unstamped source sha256: a8aca7fd3b89ba05b9c45ba49657bf30953736c9a86e38bc6d008c7f73775b90
--
package Enrolment_Pkg with SPARK_Mode is
   ...
   function Is_Live (K : Key_Record; Now : Natural) return Boolean is
     (K.Reef_Signed
      and then not K.Revoked
      and then K.Issued_At <= Now
      and then Now < K.Expires_At)
   with Post => Is_Live'Result = (K.Reef_Signed
                                  and then not K.Revoked
                                  and then K.Issued_At <= Now
                                  and then Now < K.Expires_At);
   ...
   function May_Send (K : Key_Record; Now : Natural) return Boolean is
     (Is_Live (K, Now))
   with Post => May_Send'Result = (Is_Live (K, Now));

   function May_Solve (K : Key_Record; Now : Natural) return Boolean is
     (Is_Live (K, Now)
      and then K.Role = Role_Solver
      and then K.CLA_Signed)
   with Post => May_Solve'Result = (Is_Live (K, Now)
                                    and then K.Role = Role_Solver
                                    and then K.CLA_Signed);
