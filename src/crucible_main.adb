...
--  Body: ANVIL lane, wu-crucible-main-3 round A, 2026-09-12 11:06, planner Rosie
...
--  serverInfo.name="crucible-agpl", tools=2 -- PASS, first round. Replaces the hand-driven
--  edge of 2026-09-11 (eleven rounds across two lineages, never converged; lineages
--  wu-crucible-main and wu-crucible-main-2 kept as the recorded attempt). Edition is
--  compiled in via Crucible_Edition; no Ada.Command_Line, no Ada.Environment_Variables, no
--  sockets. Self_Facts are the binary's SELF-ASSERTED facts for self_judge (door brief:
--  `facts_are_self_asserted` is owed to Reply_Text v2). Comment-stripped sha of this body
--  equals the lane-built round-A body.
with Ada.Text_IO;
with Json_Scan_Pkg;
            ...
            case V.action is
               ...
               when Json_Rpc_Frame_Pkg.Do_Call_Tool =>
                  if Frame_Facts_Pkg.Tool_Of (Line) = Frame_Facts_Pkg.T_Licence_Gate then
                     Ada.Text_IO.Put_Line (Reply_Text_Pkg.Result_Reply (Id, Tool_Run_Pkg.Licence_Gate (Crucible_Edition.Current, Line)));
                  elsif Frame_Facts_Pkg.Tool_Of (Line) = Frame_Facts_Pkg.T_Self_Judge then
                     Ada.Text_IO.Put_Line (Reply_Text_Pkg.Result_Reply (Id, Tool_Run_Pkg.Self_Judge (Self_Facts)));
                  else
