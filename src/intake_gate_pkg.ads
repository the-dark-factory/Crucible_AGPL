package Intake_Gate_Pkg with SPARK_Mode, Pure is
   ...
   with Global => null,
        Pre    => Brief'First = 1 and then Brief'Length <= Max_Brief and then Needle'First = 1 and then Needle'Length >= 1 and then Needle'Length <= 64;

   function Names_Deliverable (Brief : String) return Boolean
     is (Contains (Brief, Lit_Deliverable))
   with Global => null,
        Pre    => Brief'First = 1 and then Brief'Length <= Max_Brief;

   function Last_Non_Blank (Brief : String; Upto : Natural) return Natural
     is (if Upto = 0 then 0 elsif Brief (Upto) /= ' ' then Upto else Last_Non_Blank (Brief, Upto - 1))
   with Global => null,
        Pre    => Brief'First = 1 and then Brief'Length <= Max_Brief and then Upto <= Brief'Length,
   ...
   function Fault_Of (Brief : String) return Fault_Kind
     is ((if Is_Empty (Brief) then Empty_Brief elsif not Names_Deliverable (Brief) then No_Deliverable elsif Is_Question (Brief) then Is_A_Question else No_Fault))
   with Global => null,
        Pre    => Brief'First = 1 and then Brief'Length <= Max_Brief;
   ...
   with Global => null,
        Pre    => Brief'First = 1 and then Brief'Length <= Max_Brief,
        Post   => Passed'Result = (not Is_Empty (Brief) and then Names_Deliverable (Brief) and then not Is_Question (Brief));
