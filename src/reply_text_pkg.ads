...
--  Spec: ANVIL lane, wu-crucible-reply-text-2 round B, 2026-09-12 11:28, planner Rosie
--  qwen3.8-27b-ada:v0.3, proof_gate accepted: 243 unit checks proved, 0 unproved, 0 compile
--  errors, 0 cheat markers (round A: the seat's bound arithmetic, case 382bf241). Body:
--  the lane's body-fill (opus, candidate 2), job verified 11:28. Re-proved as built on bill:
--  level 2, -gnatwa -gnatwe, 273 checks, 0 unproved, 0 warnings. Supersedes v1 (round G,
--  09:36, crucible b814657); v1's lineage wu-crucible-reply-text kept as the record.
--  OWED (obligation 6, weak contract, unchanged from v1): Escape_Quotes escapes '"' only, not
--  '\'; its Post states First = 1 and length bounds only. Strengthen before the door relies on it.
--  Comment-stripped sha of this spec equals the forged round-B spec.
package Reply_Text_Pkg with SPARK_Mode is
   ...
   function Image_Of (N : Integer) return String
     is (if N < 0 then Integer'Image (N) else Integer'Image (N) (Integer'Image (N)'First + 1 .. Integer'Image (N)'Last))
   ...
   function Escape_Quotes (S : String) return String
     with Pre  => (S'First = 1 and then S'Length <= 4096),
          Post => (Escape_Quotes'Result'First = 1 and then Escape_Quotes'Result'Length >= S'Length and then Escape_Quotes'Result'Length <= 2 * S'Length);
