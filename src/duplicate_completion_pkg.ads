...
--  Unstamped source sha256: 3d6b5b423fe72d60dc9eb6fa8e044f511b9a7c90a40f58ec45b397bd2eb69a6a
--
package Duplicate_Completion_Pkg with SPARK_Mode is
   ...
   function Lower_Eq (A : Character; B : Character) return Boolean is
     (A = B or else
      (Is_Upper (A) and Character'Pos (A) + 32 = Character'Pos (B)) or else
      (Is_Upper (B) and Character'Pos (B) + 32 = Character'Pos (A)));
   ...
   function Function_Name_Span (T : String; Line_First : Positive; Line_Last : Positive; P : Positive; E : Positive) return Boolean is
     ...
     with Pre => T'First = 1 and then T'Length <= Max_Text and then Line_First >= 1
                 and then Line_First <= Line_Last and then Line_Last <= T'Length;
   ...
   function Has_Is_Paren (T : String; Line_First : Positive; Line_Last : Positive) return Boolean is
     ((for some I in Line_First .. Line_Last =>
        ...
        (for some Rt in Line_First .. I - 1 =>
           Token_At (T, Rt, "return") and then (for all K in Rt .. I - 1 => T (K) /= ';')) and then
   ...
   function Has_Is (T : String; Line_First : Positive; Line_Last : Positive) return Boolean is
     ((for some I in Line_First .. Line_Last =>
        ...
        (for some Rt in Line_First .. I - 1 =>
           Token_At (T, Rt, "return") and then (for all K in Rt .. I - 1 => T (K) /= ';'))))
     with Pre => T'First = 1 and then T'Length <= Max_Text and then Line_First >= 1
                 and then Line_First <= Line_Last and then Line_Last <= T'Length;
     ...
     ((for some G in 1 .. Spec'Length =>
        ...
        (for some Q in G + 9 .. Spec'Length =>
           ...
           (for some I in Q + (E - P) + 1 .. Spec'Length =>
              ...
              (for some Rt in Q + (E - P) + 1 .. I - 1 =>
                 Token_At (Spec, Rt, "return") and then (for all K in Rt .. I - 1 => Spec (K) /= ';')) and then
   ...
   function Enters (Spec : String; T : String; Line_First : Positive; Line_Last : Positive) return Boolean is
     ((for some P in Line_First .. Line_Last =>
        (for some E in P .. Line_Last =>
           Function_Name_Span (T, Line_First, Line_Last, P, E) and then Completed_In_Spec (Spec, T, P, E))))
     with Pre => Spec'First = 1 and then Spec'Length <= Max_Text and then T'First = 1 and then T'Length <= Max_Text
   ...
   function Name_First_Of (T : String; Line_First : Positive; Line_Last : Positive) return Positive
     with Pre => T'First = 1 and then T'Length <= Max_Text and then Line_First >= 1
                 and then Line_First <= Line_Last and then Line_Last <= T'Length
                 and then (for some P in Line_First .. Line_Last =>
                             (for some E in P .. Line_Last => Function_Name_Span (T, Line_First, Line_Last, P, E))),
          Post => Name_First_Of'Result in Line_First .. Line_Last
                  and then (for some E in Name_First_Of'Result .. Line_Last =>
                              Function_Name_Span (T, Line_First, Line_Last, Name_First_Of'Result, E))
     ...
     (case S.state is
        ...
        when Full_Form =>
          (state      => (if End_Line_Of (T, Line_First, Line_Last, S.name_first, S.name_last) then Outside else Full_Form),
           name_first => S.name_first, name_last => S.name_last, blank => True))
     with Pre => Spec'First = 1 and then Spec'Length <= Max_Text and then T'First = 1
                 and then T'Length <= Max_Text and then Line_First >= 1 and then Line_First <= Line_Last
                 and then Line_Last <= T'Length
