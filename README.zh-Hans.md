...
一座工厂的、经过证明的决策内核——这座工厂将把写下来的规格说明变成带有机器可验证证明的 Ada/SPARK，并拒绝交付任何它无法证明的东西。

**坦白说，这个版本是什么：**它目前还不接受规格说明，也不锻造任何东西。它给你的是工厂经过证明的决策核心——你可以在自己的硬件上重新证明它们，而不必相信我们——以及一个 MCP 入口，其唯一可用的工具是许可证闸门。没有预编译的二进制文件。发布说明准确列出了这里有什么、没有什么。
